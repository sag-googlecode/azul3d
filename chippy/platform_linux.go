...
// For converting X11 keysym -> unicode
#include "keysym2ucs.h"

#cgo LDFLAGS: -lX11 -lXxf86vm -lGL -lXi -lXcursor
...
XKeyEvent XEvent_xkey(XEvent event) {
    return event.xkey;
}
...
XGenericEventCookie XGenericEventCookie_xcookie(XEvent event) {
    return event.xcookie;

    //XGenericEventCookie* ptr = (XGenericEventCookie*)malloc(sizeof(XGenericEventCookie));
    //*ptr = event.xcookie;
    //return ptr;
    //
    //XGenericEventCookie* ptr = &event.xcookie;
    //return ptr;
    //return (XGenericEventCookie*)(&event.xcookie);
}
...
void setCursorImagePixel(XcursorImage* image, int index, unsigned int pixel) {
    image->pixels[index] = pixel;
}

void setUintPtrAtIndex(unsigned int* buffer, int index, unsigned int value) {
    buffer[index] = value;
}
*/
import "C"
...
type c_XGCValues C.XGCValues
type c_XColor C.XColor
type c_Time C.Time
type c_XIconSize C.XIconSize

type c_XIDeviceEvent C.XIDeviceEvent
type c_XcursorImage C.XcursorImage
...
func (c c_Drawable) C() C.Drawable {
	return C.Drawable(c)
}
...
func (c c_Time) C() C.Time {
	return C.Time(c)
}

func (c c_KeySym) C() C.KeySym {
    return C.KeySym(c)
}

func (c c_KeyCode) C() C.KeyCode {
    return C.KeyCode(c)
}

func (c c_XKeyEvent) C() C.XKeyEvent {
    return C.XKeyEvent(c)
}
...
func (e c_XEvent) xkey() c_XKeyEvent {
	return (c_XKeyEvent)(C.XEvent_xkey(e.C()))
}
...
func (e c_XEvent) xcookie() *c_XGenericEventCookie {
	...
	v := C.XGenericEventCookie_xcookie(e.C())
	return (*c_XGenericEventCookie)(&v)
}
...
var c_GrabModeAsync = C.GrabModeAsync
var c_GrabSuccess = C.GrabSuccess
var c_CurrentTime c_Time = C.CurrentTime
...
var c_XI_RawMotion int = C.XI_RawMotion
var c_XI_KeyPress int = C.XI_KeyPress
var c_XI_KeyRelease int = C.XI_KeyRelease
...
var c_PropModeReplace int = C.PropModeReplace
var c_GXclear C.int = C.GXclear
var c_GCFunction uint64 = C.GCFunction
...
func c_XUnmapWindow(display *c_Display, window c_Window) {
    C.XUnmapWindow(display.C(), window.C())
}
...
func c_XChangeProperty(display *c_Display, window c_Window, property, _type c_Atom, format, mode int, data c_Pointer, nelements int) error {
...
func c_XDefineCursor(display *c_Display, window c_Window, cursor c_Cursor) {
    C.XDefineCursor(display.C(), window.C(), cursor.C())
}

func c_XCreatePixmap(display *c_Display, d c_Drawable, width, height, depth uint) c_Pixmap {
    return c_Pixmap(C.XCreatePixmap(display.C(), d.C(), C.uint(width), C.uint(height), C.uint(depth)))
}

func c_XCreateGC(display *c_Display, d c_Drawable, valuemask uint64, values *c_XGCValues) c_GC {
    return c_GC(C.XCreateGC(display.C(), d.C(), C.ulong(valuemask), (*C.XGCValues)(values)))
}

func c_XFillRectangle(display *c_Display, d c_Drawable, gc c_GC, x, y int, width, height uint) {
    C.XFillRectangle(display.C(), d.C(), C.GC(gc), C.int(x), C.int(y), C.uint(width), C.uint(height))
}

func c_XCreatePixmapCursor(display *c_Display, source, mask c_Pixmap, foreground_color, background_color *c_XColor, x, y uint) c_Cursor {
    return c_Cursor(C.XCreatePixmapCursor(display.C(), source.C(), mask.C(), (*C.XColor)(foreground_color), (*C.XColor)(background_color), C.uint(x), C.uint(y)))
}

func c_XFreePixmap(display *c_Display, pixmap c_Pixmap) {
    C.XFreePixmap(display.C(), pixmap.C())
}

func c_XFreeGC(display *c_Display, gc c_GC) {
    C.XFreeGC(display.C(), C.GC(gc))
}

func c_XGrabPointer(display *c_Display, grab_window c_Window, owner_events bool, event_mask uint, pointer_mode, keyboard_mode int, confine_to c_Window, cursor c_Cursor, time c_Time) int {
    return int(C.XGrabPointer(display.C(), grab_window.C(), c_bool(owner_events), C.uint(event_mask), C.int(pointer_mode), C.int(keyboard_mode), confine_to.C(), cursor.C(), time.C()))
}

func c_XUngrabPointer(display *c_Display, time c_Time) {
    C.XUngrabPointer(display.C(), time.C())
}

func c_XGetIconSizes(display *c_Display, w c_Window) ([]*c_XIconSize, error) {
    var count C.int
    var list *C.XIconSize

    slice := []*c_XIconSize{}

    if C.XGetIconSizes(display.C(), w.C(), &list, &count) == 0 {
        return slice, errors.New("XGetIconSizes() failed")
    }

    for i := 0; i < int(count); i++ {
        sz := uintptr(unsafe.Pointer(list)) + uintptr(i)
        size := (*C.XIconSize)(unsafe.Pointer(sz))
        slice = append(slice, (*c_XIconSize)(size))
    }
    return slice, nil
}

func c_XSetWMHints(display *c_Display, w c_Window, wmhints *c_XWMHints) {
    C.XSetWMHints(display.C(), w.C(), (*C.XWMHints)(wmhints))
}

func c_XGetWMHints(display *c_Display, w c_Window) *c_XWMHints {
    return (*c_XWMHints)(C.XGetWMHints(display.C(), w.C()))
}

func c_XDisplayKeycodes(display *c_Display) (int, int) {
    var min_keycodes, max_keycodes C.int
    C.XDisplayKeycodes(display.C(), &min_keycodes, &max_keycodes)
    return int(min_keycodes), int(max_keycodes)
}

func c_XGetKeyboardMapping(display *c_Display, first_keycode c_KeyCode, keycode_count int) (int, []c_KeySym) {
    var keysyms_per_keycode C.int
    var sym *C.KeySym = C.XGetKeyboardMapping(display.C(), first_keycode.C(), C.int(keycode_count), &keysyms_per_keycode)
    defer C.XFree(unsafe.Pointer(sym))

    slice := make([]c_KeySym, keycode_count-1)
    for i := 0; i < keycode_count; i++ {
        address := uintptr(unsafe.Pointer(sym)) + uintptr(i)
        element := (*C.KeySym)(unsafe.Pointer(address))
        slice = append(slice, (c_KeySym)(*element))
    }
    return int(keysyms_per_keycode), slice
}

func c_XAutoRepeatOn(display *c_Display) {
    C.XAutoRepeatOn(display.C())
}

func c_XAutoRepeatOff(display *c_Display) {
    C.XAutoRepeatOff(display.C())
}
