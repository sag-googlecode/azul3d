	...
	*/
	...
	/*
		// Write enums
			...
			for _, t := range r.Types.Type {
				log.Printf("Definition: %q\n", t.Definition)
				if len(t.Name) > 0 {
					log.Printf("  Name: %q\n", t.Name)
				} else {
					log.Printf("  NameAttr: %q\n", t.NameAttr)
				}
				if len(t.Comment) > 0 {
			...
			for _, g := range r.Groups.Group {
				log.Printf("Group: %q\n", g.Name)
				for _, e := range g.Enum {
					log.Printf("  %q\n", e.Name)
				}
			}

			for _, es := range r.Enums {
				log.Printf("Namespace: %q\n", es.Namespace)
				if len(es.Group) > 0 {
					log.Printf("  Group: %q\n", es.Group)
				}
				if len(es.Type) > 0 {
			...
			for _, e := range r.Extensions.Extension {
				log.Printf("Name: %q\n", e.Name)
				log.Printf("Supported: %q\n", e.Supported)
