...
*/
func char_callback(character int) {
    counter += 1
    fmt.Printf("callback\n")
    //fmt.Printf("%08x at %0.3f: Character 0x%04x (%s) input\n", counter, glfw.Time(), character, string(character))
    glfw.Time()
    fmt.Printf("post-callback\n")
}
...
func main() {
    ...
    window, err := glfw.CreateWindow(0, 0, glfw.WINDOWED, "Event Linter", nil)
    if err != nil {
        panic(err.Error())
    }
...
//    window.SetKeyCallback(key_callback)
    ...
    fmt.Printf("Window size should be %dx%d\n", width, height)
    ...
    fmt.Printf("Key repeat should be %s\n", kr)
    ...
    fmt.Printf("System keys should be %s\n", sk)

    fmt.Println("Main loop starting")

    for window.Param(glfw.CLOSE_REQUESTED)==0 {
        glfw.WaitEvents()
    }
}
